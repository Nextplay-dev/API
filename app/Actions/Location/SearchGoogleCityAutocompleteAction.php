...
class SearchGoogleCityAutocompleteAction
{
    public function handle(string $query): array
    {
        ...
        if (! $response->successful()) {
            throw new RuntimeException('Failed to fetch city suggestions.');
        }

        $data = $response->json();

        if (($data['status'] ?? null) !== 'OK' && ($data['status'] ?? null) !== 'ZERO_RESULTS') {
            throw new RuntimeException($data['error_message'] ?? 'Failed to fetch city suggestions.');
        }
